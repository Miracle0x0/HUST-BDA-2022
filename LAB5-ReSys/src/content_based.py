    ...
    def recommend(self, user_id: int, k: int):
        ...
        for i in range(len(self.movies)):
            ...
            if idx not in rated_id:
                rec_movies[(idx, title)] = self.get_predict_score(rated_score, rated_id, self.id_to_index[idx])
        # 排序
...
class MinHashContentRecommendSystem(ContentBasedRecommendSystem):
    ...
    def get_signature_mat(self, hash_func_cnt: int = 20):
        ...
        for i in range(hash_func_cnt):
            ...
            self.signature_mat[i] = np.array([np.where(tm[:, j] == 1)[0][0] for j in range(tm.shape[1])])
...
if __name__ == "__main__":
    ...
    base_recommender = ContentBasedRecommendSystem(utility_matrix, movies, tfidf_matrix)
